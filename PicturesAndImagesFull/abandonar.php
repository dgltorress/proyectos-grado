                ...
                <section> <!--Últimos álbumes-->
                    ...
                    <div class="fotosRecientes">
                        ...
                        <p>
                            <?php
                                if($hayDatos){
                                        $total = @mysqli_query( $conexion,
                                        'SELECT COUNT(f.id) AS Total FROM fotos AS f
                                        inner JOIN albumes AS a ON a.id = f.album
                                        WHERE a.usuario = '.$datosUsuario['id'].'
                                        LIMIT 20' ) OR die( "No se ha podido realizar la consulta." );
                                        while( $foto = @mysqli_fetch_assoc( $total ) ) total( $foto );
                    ...
                    <form id="relay" method="POST" action="php/borrar.php" >
                        <label>Introduce tu contraseña</label>
                        <input type='password' name='password'>
                        <input type='hidden' name='usr' value=<?php echo $datosUsuario['nombre']?>>
                    </form>
